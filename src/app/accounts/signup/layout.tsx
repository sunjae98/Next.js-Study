import { ReactNode } from "react";
import Image from "next/image";

type Props = { children: ReactNode };
export default function SignupLayout({ children }: Props) {
  return (
    <main className="flex">
      <div className="h-dvh flex flex-col flex-1 bg-pointColor items-center justify-center">
        <Image
          className="mb-4"
          src="data:image/png;base64,"
          alt="/*elice*/"
          width={108}
          height={32}
        />
        <p className="text-white text-2xl font-bold">
          엘리스에서 Run it, Learn it
        </p>
        <Image
          className="mt-20"
          src="https://cdn-front-door.elice.io/accounts/static/media/signup_run.02df36bb0dac45b7.png"
          alt="learn in run it"
          width={200}
          height={123}
        />
      </div>
      <div className="h-dvh flex flex-col flex-1 bg-[#F5F5F5] items-center justify-center">
        {children}
      </div>
    </main>
  );
}
